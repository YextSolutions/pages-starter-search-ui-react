...
const filters = /* @__PURE__ */ new Set(["./src/bundler.js"]);
const files = glob.sync("./src/**/*.*").filter(testFilter).filter((f) => !filters.has(f)).filter((f) => !f.endsWith(".md"));
...
pluginFiles.map(
  (filepath) => copyFileSync(filepath, `${pluginOutputPath}${path.basename(filepath)}`)
);
try {
